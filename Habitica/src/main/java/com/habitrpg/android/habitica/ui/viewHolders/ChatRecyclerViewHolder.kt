    ...
    fun bind(
        ...
        if (wasSent) {
            ...
            if (user?.username != null) {
                ...
                binding.sublineTextview.text =
                    "${user.formattedUsername} ∙ ${msg.timestamp?.getAgoString(res)}"
            } else {
            ...
            if (msg.username != null) {
                @SuppressLint("SetTextI18n")
                binding.sublineTextview.text =
                    "${msg.formattedUsername} ∙ ${msg.timestamp?.getAgoString(res)}"
            } else {
                binding.sublineTextview.text = msg.timestamp?.getAgoString(res)
            }
        }
        when (binding.userLabel.tier) {
            8 -> {
                ...
                binding.modView.background =
                    ContextCompat.getDrawable(context, R.drawable.pill_bg_blue)
            ...
            9 -> {
                ...
                binding.modView.background =
                    ContextCompat.getDrawable(context, R.drawable.pill_bg_purple_300)
                binding.modView.setScaledPadding(context, 12, 4, 12, 4)
            }
        ...
        if (isExpanded) {
            ...
            binding.replyButton.visibility =
                if (chatMessage?.isInboxMessage == true) View.GONE else View.VISIBLE
        } else {
        ...
        if (flagCount > 0 && user?.hasPermission(Permission.MODERATOR) == true) {
            binding.flagCountTextview.text =
                if (flagCount == 10) {
