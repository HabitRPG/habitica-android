    ...
    constructor(
        ...
        fun equipCustomization(item: SetupCustomization) {
            ...
            when (item.category) {
                SetupCustomizationRepository.CATEGORY_BODY -> {
                    currentUser.preferences?.shirt = item.key
                }

                SetupCustomizationRepository.CATEGORY_HAIR -> {
                    when (item.subcategory) {
                        SetupCustomizationRepository.SUBCATEGORY_COLOR -> {
                            currentUser.preferences?.hair?.color = item.key
                        }

                        SetupCustomizationRepository.SUBCATEGORY_BANGS -> {
                            currentUser.preferences?.hair?.bangs = item.key.toIntOrNull() ?: 0
                        }

                        SetupCustomizationRepository.SUBCATEGORY_PONYTAIL -> {
                            currentUser.preferences?.hair?.base = item.key.toIntOrNull() ?: 0
                        }
        ...
        fun getActiveCustomization(
            ...
            return when (category) {
                SetupCustomizationRepository.CATEGORY_BODY -> {
                    currentUser?.preferences?.shirt
                }

                SetupCustomizationRepository.CATEGORY_HAIR -> {
                    when (subcategory) {
                        SetupCustomizationRepository.SUBCATEGORY_COLOR -> {
                            currentUser?.preferences?.hair?.color
                        }
                ...
                SetupCustomizationRepository.CATEGORY_SKIN -> {
                    currentUser?.preferences?.skin
                }

                SetupCustomizationRepository.CATEGORY_EXTRAS -> {
                    when (subcategory) {
                        SetupCustomizationRepository.SUBCATEGORY_WHEELCHAIR -> {
                            currentUser?.preferences?.chair
                        }
        ...
        suspend fun saveSetup(context: Context) {
            userRepository.updateUser(
                mapOf(
                    ...
                    "preferences.hair.color" to
                        _user.value
                            ?.preferences
                            ?.hair
                            ?.color,
                    "preferences.hair.bangs" to
                        _user.value
                            ?.preferences
                            ?.hair
                            ?.bangs,
                    "preferences.hair.base" to
                        _user.value
                            ?.preferences
                            ?.hair
                            ?.base,
                    "preferences.hair.flower" to
                        _user.value
                            ?.preferences
                            ?.hair
                            ?.flower,
