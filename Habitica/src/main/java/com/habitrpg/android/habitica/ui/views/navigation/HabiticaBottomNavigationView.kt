...
class HabiticaBottomNavigationView
@JvmOverloads
constructor(
    context: Context,
    ...
    init {
        ...
        binding.addButton.setOnTouchListener { _, event ->
            if (event.action == MotionEvent.ACTION_DOWN) {
                ...
                animY.start()
                val animXBackground =
                    ObjectAnimator.ofFloat(binding.addButtonBackground, "scaleX", 1f, 0.9f)
                animXBackground.duration = 100
                animXBackground.interpolator = LinearInterpolator()
                animXBackground.start()
                val animYBackground =
                    ObjectAnimator.ofFloat(binding.addButtonBackground, "scaleY", 1f, 0.9f)
    ...
    private fun animateButtonTap() {
        ...
        animY.start()
        val animXBackground =
            ObjectAnimator.ofFloat(binding.addButtonBackground, "scaleX", 0.9f, 1f)
        animXBackground.duration = 600
        animXBackground.interpolator = BounceInterpolator()
        animXBackground.start()
        val animYBackground =
            ObjectAnimator.ofFloat(binding.addButtonBackground, "scaleY", 0.9f, 1f)
    ...
    private fun showSubmenu() {
        ...
        for (taskType in listOf(TaskType.HABIT, TaskType.DAILY, TaskType.TODO, TaskType.REWARD)) {
            ...
            view.alpha = 0f
            view.scaleY = 0.7f
            ViewCompat.animate(view).alpha(1f).setDuration(250.toLong()).startDelay =
                (100 * pos).toLong()
            ViewCompat.animate(view).scaleY(1f).setDuration(250.toLong()).startDelay =
                (100 * pos).toLong()
            pos -= 1
        }
        var widestWidth = 0
    ...
    private fun hideSubmenu() {
        ...
        for (view in binding.submenuWrapper.children) {
            view.alpha = 1f
            view.scaleY = 1f
            ViewCompat.animate(view).alpha(0f).setDuration(200.toLong()).startDelay =
                (150 * pos).toLong()
            ViewCompat.animate(view).scaleY(0.7f).setDuration(250.toLong())
                .setStartDelay((100 * pos).toLong())
                .withEndAction { binding.submenuWrapper.removeView(view) }
            pos += 1
        }
    }
