...
class StableViewModel
    ...
    constructor(
        ...
        val eggs: LiveData<Map<String, Egg>> =
            inventoryRepository
                .getItems(Egg::class.java)
                .map {
        ...
        val ownedItems: LiveData<Map<String, OwnedItem>> =
            inventoryRepository.getOwnedItems(true).asLiveData()
        ...
        private fun mapAnimals(
            unsortedAnimals: List<Animal>,
            ownedAnimals: Map<String, OwnedObject>,
        ): ArrayList<Any> {
            ...
            for (animal in unsortedAnimals) {
                val identifier =
                    if (animal.animal.isNotEmpty() && (animal.type != "special" && animal.type != "wacky")) animal.animal else animal.key
                ...
                if (animal.type == "premium") {
                    ...
                    if (!((lastAnimal.type == "special") && lastAnimal.numberOwned == 0) &&
                        !items.contains(
                            lastAnimal,
                        )
                    ) {
                ...
                if (animal.type != lastSection?.key && animal.type != "premium") {
                    if (items.isNotEmpty() && items[items.size - 1].javaClass == StableSection::class.java) {
                ...
                lastSection?.totalCount = lastSection.totalCount + 1
                if (isOwned) {
                    lastAnimal.numberOwned += 1
                    lastSection?.ownedCount = lastSection.ownedCount + 1
                }
            }
