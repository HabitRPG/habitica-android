        ...
        fun handleOpenedByNotification(
            ...
            scope
                .launch(context = Dispatchers.Main) {
                    when (identifier) {
                        PushNotificationManager.PARTY_INVITE_PUSH_NOTIFICATION_KEY -> {
                            openNoPartyScreen()
                        }

                        PushNotificationManager.QUEST_BEGUN_PUSH_NOTIFICATION_KEY -> {
                            openPartyScreen()
                        }

                        PushNotificationManager.QUEST_INVITE_PUSH_NOTIFICATION_KEY -> {
                            openPartyScreen()
                        }
                        ...
                        PushNotificationManager.CHANGE_USERNAME_PUSH_NOTIFICATION_KEY -> {
                            openSettingsScreen()
                        }
                        ...
                        PushNotificationManager.G1G1_PROMO_KEY -> {
                            openGiftOneGetOneInfoScreen()
                        }
