...
constructor(
    ...
    fun filterCount(type: TaskType?): Int = this.tags.size + if (isTaskFilterActive(type)) 1 else 0

    fun isFiltering(type: TaskType?): Boolean = filterCount(type) > 0

    private fun isTaskFilterActive(type: TaskType?): Boolean {
        return activeFilters[type] != null && if (TaskType.TODO == type) {
    ...
    private fun isFiltered(
        ...
        return task.containsAllTagIds(tags) && (!(activeFilter != null && activeFilter != Task.FILTER_ALL) || when (activeFilter) {
            Task.FILTER_ACTIVE -> {
    ...
    fun getTaskFilterPreference(type: TaskType): String =
        sharedPreferences.getString("filter_${type.value}", Task.FILTER_ALL)
            ?: if (TaskType.TODO == type) Task.FILTER_ACTIVE else Task.FILTER_ALL
        ...
        if (unfilteredData.isNotEmpty()) {
            ...
            if (activeFilter != null && activeFilter != Task.FILTER_ALL) {
                when (activeFilter) {
                    ...
                    Task.FILTER_WEAK -> {
                        query = query.lessThan("value", 1.0)
                    }

                    Task.FILTER_STRONG -> {
                        query = query.greaterThanOrEqualTo("value", 1.0)
                    }
    ...
    override fun canScoreTask(task: Task): Boolean {
        return !task.isGroupTask || task.isAssignedToUser(userViewModel.userID) || task.group?.assignedUsers?.isEmpty() != false
    }
    ...
    override fun assignedTextForTask(
        ...
        if (assignedUsers.contains(userViewModel.userID)) {
            ...
            if (assignedUsers.size == 1) {
                userViewModel.currentTeamPlanMembersData.value
                    ?.firstOrNull { it.id == assignedUsers.first() }
                    ?.displayName
                    ?: ""
            } else {
                resources.getQuantityString(
