...
constructor(
    ...
    internal fun refreshData(onComplete: () -> Unit) {
        viewModelScope.launch(ExceptionHandler.coroutine()) {
            ...
            } else userRepository.retrieveTeamPlan(ownerID.value ?: "")
    ...
    fun getFilterSet(type: TaskType): MutableLiveData<Triple<String?,
            String?, List<String>>>? = filterSets[type]
    ...
    fun addActiveTag(tagID: String) {
        ...
        if (!tags.contains(tagID)) tags.add(tagID)
    ...
    private fun isTaskFilterActive(type: TaskType?): Boolean {
        ...
        if (activeFilters[type] == null) return false
    ...
    override suspend fun canAddTasks(): Boolean {
        ...
        if (isPersonalBoard) return true
