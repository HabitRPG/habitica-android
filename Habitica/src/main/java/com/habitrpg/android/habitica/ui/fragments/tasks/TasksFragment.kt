...
@AndroidEntryPoint
class TasksFragment :
    BaseMainFragment<FragmentViewpagerBinding>(),
    SearchView.OnQueryTextListener,
    HabiticaBottomNavigationViewListener {
    @Inject
    ...
    private fun getTaskTypeFromTabPosition(position: Int?): TaskType? =
        when (position) {
            0 -> TaskType.HABIT
            1 -> TaskType.DAILY
            2 -> TaskType.TODO
